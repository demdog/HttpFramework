...
import com.apigj.android.httpframework.controller.connect.serviceTask.PutTask
...
@Keep
class FilePutService(context: Context, url: String, filePath: String):IService, IRemoteHandler {
    ...
    @Keep
    fun getCachedFileUploadService(context: Context): List<FilePutService> {
        val res = ArrayList<FilePutService>()
        val cufb = CacheUploadFileDB(context)
        try {
            val files = cufb.queryDBData()
            if (files != null) {
                for (file in files!!) {
                    res.add(FilePutService(context, file.uploadUrl, file.filePath))
                }
            }
        } finally {
            if (cufb != null) {
